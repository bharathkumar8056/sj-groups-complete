  ...
  const products = [
    {
      ...
      category: "Salt",
      link: "/products/bamboo-salt-3x",
      image: "/assets/bamboo-salt-8x.jpg",
      description: "Premium 3x roasted bamboo salt - perfect balance of purity and mineral richness for daily culinary and wellness use.",
    },
    {
          ...
          {products.map((product) => (
            <Link key={product.name} href={product.link}>
              <Card className="card-premium p-6 h-full flex flex-col items-center justify-center text-center cursor-pointer group">
                ...
                {/* If a product has variants (collection), show description + bold variant bullets; otherwise show description */}
                {product.variants ? (
                  <>
